...
5. **Update or add the matching DTO.** In `ios/Venn/Models/` (or the relevant feature folder), update the `Decodable` struct to match the new shape. Commit it alongside the migration.
