...
The Swift type bindings are generated by `supabase-swift` at the call site (the client infers shapes from the `Decodable` types you pass in `.execute(returning:)`). DTO types live in `ios/Venn/Models/` (or in the relevant feature folder when used by exactly one feature) — see [`ARCHITECTURE.md`](./ARCHITECTURE.md#naming-conventions).

For the row shape itself, [`ios/Venn/Models/SupabaseSchema.swift`](../ios/Venn/Models/SupabaseSchema.swift) is generated from the live database by [`scripts/gen-types.mjs`](../scripts/gen-types.mjs). Each table becomes an enum namespace (`ProfilesSchema`) containing a `Row: Codable` struct with all columns typed correctly. Hand-written domain models (e.g. [`UserProfile`](../ios/Venn/Models/UserProfile.swift)) can map to/from these row types in the matching service. Regenerate with `make codegen` (or `npm run db:types`) after every schema change — the file is committed.
...
| `npm run db:types`       | Regenerate `ios/Venn/Models/SupabaseSchema.swift` from the linked DB. Requires `SUPABASE_DB_URL` in `.env`. Run after schema changes. |
...
5. **Regenerate the schema bindings.** `make codegen` (or `npm run db:types`) — rewrites `ios/Venn/Models/SupabaseSchema.swift` from the live DB. Commit the diff alongside the migration. If you also have hand-written domain models that mirror the changed table, update those in the same PR.
6. **Write the application code.** Service, view-model, view — using the updated DTO.
7. **Verify.** `make verify`.
8. **Open the PR.** Reviewer sees migration + DTO + code in one place.
...
The first migration (`20260425120000_init.sql`) is the canonical schema with idempotency guards (`IF NOT EXISTS`, `DROP POLICY IF EXISTS`). It's safe to apply against a Supabase project that was already initialized — every statement is a no-op when the object already exists, except for the sanitization CHECK constraints which will be added.
